...
class Paginator {
    ...
    _create(main) {
        let ini = (this._beginPage =
            this.maxPages * Math.floor((this.page - 1) / this.maxPages) + 1);
    ...
    pageList() {
        if (this.totalPages > this.maxPages) {
            let selectPage = (this._selectPage = this._main
                .create("select")
    ...
    updatePages() {
        ...
        if (this.totalPages > this.maxPages) {
            this._main.query("select").value =
                (Math.ceil(this.page / this.maxPages) - 1) * this.maxPages + 1;
        }
        this.updatePageList();
    }
...
export class Grid2 {
    ...
    _search = (index: number) => {
        return true;
    };
    _filter = (index: number) => {
        return true;
    };
    ...
    _create(main: any) {
        ...
        main
            .create({ tagName: "div", className: "caption" })
        ...
        this._data_grid = hiddenDiv.create({
            tagName: "input",
            type: "hidden",
            name: "__data_",
        });
    ...
    createRow(data) {
        ...
        for (let x in this.fields) {
            ...
            input =
                field.input === "hidden" || this.type === "default"
                    ? "input"
                    : field.input;
    ...
    createRowORG(data) {
        ...
        if (cell) {
            hiddenFields.append(
                I.create("input", {
                    type: "hidden",
                    name: "__mode_" + this._rowLength,
                    value: data["__mode_"],
                    dataset: { name: "__mode_" },
                })
            );
            hiddenFields.append(
                I.create("input", {
                    type: "hidden",
                    name: "__id_" + this._rowLength,
                    value: data["__id_"],
                    dataset: { name: "__id_" },
                })
            );
            hiddenFields.append(
                I.create("input", {
                    type: "hidden",
                    name: "__index_" + this._rowLength,
                    value: this._rowLength,
                    dataset: { name: "__index_" },
                })
            );
            cell.append(hiddenFields);
        }
    ...
    saveAt(index, data) {
        if (this.type === "default") {
            let value = null;
            let cells = this._tbody.queryAll(
                `.body-row[data-index='${index}'] td[data-name]`
            );
    ...
    saveAtORG(index, data) {
        let cells = this._main.queryAll(
            ".body-row[data-index='" + index + "'] > [data-name]"
        );
